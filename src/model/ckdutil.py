...
def PPR(node2neigh_list, alpha=0.15):
    ...
    for node2neigh in node2neigh_list:
        ...
        for node in range(len(node2neigh)):
            neighs = np.zeros(shape=len(node2neigh), dtype=np.int64)
...
def output(args, embeddings, id2name, need_handle=True):
    ...
    output_path = f'./output/embedding/CKD/{args.dataset}_{args.target_node}.txt'
    folder_path = os.path.dirname(output_path)
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    with open(output_path, 'w') as file:
        file.write(
            f'{len(id2name)}\t{args.size}\n')
        count = -1
        for nid, name in id2name.items():
            if count == -1:
                count = int(name)
            name -= count
            name = args.target_node+str(name)
